...
export async function renderHome(root) {
  ...
  root.innerHTML = `<div class="view home">
    ...
    <section class="sec" id="all">
      <button class="allcta reveal" id="allBtn" aria-label="전체 공고 보기 — 로그인 화면으로 이동">
        <span class="glow"></span>
        <span class="inner">
          <span class="k meta">ALL NOTICES · 조건 필터 · 근거 질문</span>
          <span class="ttl"><b>전체</b>공고<br>보기</span>
          <span class="foot"><span class="ftxt">로그인하면 조건 필터로 전체 공고를 좁혀 보고, 선택한 공고에 대해 질문하며 원문 근거를 확인할 수 있습니다.</span>
          <span style="display:flex;align-items:center;gap:18px"><span class="count" id="allCount"></span><span class="go">${icons.arrowUR}</span></span></span>
        </span>
      </button>
    </section>
  ...
  $('#allBtn').addEventListener('click', () => user ? go('#/app') : toLogin());
  ...
  $('#allCount').textContent = `공개 공고 ${notices.length}건`;
